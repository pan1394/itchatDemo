...
public class DemoHandler implements IMsgHandlerFace {
	...
	public String textMsgHandle(BaseMsg msg) { 
		...
		if(msg.getType().equals(MsgTypeEnum.CALLBACK.getType())) {
			...
			String userName = base.getFromUserName();
			String text = base.getText();
			MessageTools.sendMsgById(String.format("%s撤回了消息：%s", MyUtils.getDisplayedNameByUserName(userName) , text), "filehelper");
		}else {
			messages.put(msg.getMsgId(), msg);
		}
		
		logger.info("{}:{}", MyUtils.getDisplayedNameByUserName(msg.getFromUserName()), msg.getText()); 
		return null;
	}
